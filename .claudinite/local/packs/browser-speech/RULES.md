...
handlers, mic release, which constraints a mic capture may even ask for, whether an interim
hypothesis is told apart from a transcript, and whether an error mapping is total — are this
pack's six checks; what follows is only what no check can decide for you.
...
to the caller. (That a recognizer asking for interim results tells them apart from finals at all
is the `stt-interim-results-gated` check; the pause monitor around it is not.)
