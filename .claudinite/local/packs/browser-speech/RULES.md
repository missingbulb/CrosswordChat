...
is total, and whether a handler that asked for interim results distinguishes them — are this
...
to the caller. (That a handler which enables interim results distinguishes them at all — the
`isFinal` gate — is the `stt-interim-results-gated` check. Whether a pause monitor exists, and
what it does with the half-heard input, is not.)
