## 2026-08-07 · born · the host's idle timer, in the lifecycle section (#130)
...
- **Mechanism:** a bullet of the host-lifecycle section in `local/host-page-adaptation`'s RULES.md.
...
- **Mechanism:** a RULES.md rule in `local/crossword-chat`, triggered on "Keeping a voice session
  alive on nytimes.com"; the general idle-nudge rule is canon `host-page`'s.
