## 2026-08-07 · born · put back borrowed host state, and degrade when unreadable (#130)
...
- **Mechanism:** a RULES.md rule in `local/crossword-chat`, triggered on "Borrowing pencil mode for
  a write".
- **Landed:** #407 (Closes #396).

## 2026-09-13 · reworded · stripped to the control and its REQ ids (#422)
- **Source:** the growth-dedup run (#418) against canon `host-page` and `web-speech`, both of which
  changed in the window.
- **Reason:** canon `host-page`'s "Toggling a host control" rule now carries the unreadable-state
  reasoning this bullet restated.
