...
describe('persisted settings (REQ-NAV-012)', () => {
  test('REQ-NAV-012: settings round-trip through chrome.storage.sync', async () => {
    ...
    expect(await loadSettings()).toEqual({ strategy: 'most-filled', rate: 1.7, biasing: 'commands' });
  });
  ...
  test('REQ-NAV-012: unknown stored values are sanitized to the defaults', async () => {
    ...
    expect(await loadSettings()).toEqual({ strategy: 'list-order', rate: 1.3, biasing: 'commands' });
    expect(sanitizeSettings({})).toEqual(DEFAULT_SETTINGS);
    expect(sanitizeSettings(null)).toEqual(DEFAULT_SETTINGS);
  });

  test('REQ-SPCH-011: the biasing mode round-trips; default is commands; off is an explicit choice', async () => {
    fakeChromeStorage();
    ...
    expect(sanitizeSettings({ biasing: 'bogus' }).biasing).toBe('commands'); // unknown → the default
    expect(sanitizeSettings({}).biasing).toBe('commands');
    expect(DEFAULT_SETTINGS.biasing).toBe('commands');
  });
