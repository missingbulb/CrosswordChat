...
| D11 | TTS-vs-listener conflict handled by the app-level echo guard, **not** by constraining the recognizer's capture | `webkitSpeechRecognition` owns its capture and exposes no hook for `MediaTrackConstraints`; Chrome's default AEC (playout loopback) already attenuates in-browser TTS but misses `chrome.tts`'s OS-rendered output, so the string-match echo guard (REQ-SPCH-005) stays necessary. `echoCancellation`/`noiseSuppression` are requested on the one capture we own (the mic preflight, REQ-SPCH-003) and the applied AEC state is logged for diagnosis. `suppressLocalAudioPlayback`/`restrictOwnAudio` are `getDisplayMedia` screen-capture constraints — inapplicable to a mic+TTS setup | The Web Speech API gains a stream/constraints hook, or TTS shifts to an in-browser playout path so Chrome's default AEC can reference and cancel it — `speechSynthesis` already renders through Chrome's mix (so it is already largely cancelled); `chrome.tts`'s OS-rendered output is the part the loopback misses. Note this is a *playout-path* choice, not a Web Audio one: neither `chrome.tts` nor `speechSynthesis` exposes a stream we could route through Web Audio, so Web Audio cannot carry TTS here (it stays confined to the ready ping) |
